...
lib.callback.register('randol_pizzajob:server:Payment', function(source)
    ...
    lib.callback.await('basic:ShowNotify', source, ('Vous avez reçu $%s. Plus de livraisons restantes, retournez le véhicule.'):format(players[src].payment))
        return true
    end
    
    lib.callback.await('basic:ShowNotify', source, ('Vous avez reçu $%s. Livraisons restantes : %s'):format(players[src].payment, #players[src].locations))
