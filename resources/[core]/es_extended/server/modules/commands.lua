...
ESX.RegisterCommand({ "clearall", "clsall" }, "owner", function(xPlayer)
...
if not Config.CustomInventory then
    ESX.RegisterCommand("refreshitems", "owner", function(xPlayer)
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if args.group == "superowner" then
            args.group = "owner"
            print("[^3WARNING^7] ^5Superowner^7 detected, setting group to ^5owner^7")
        end
...
ESX.RegisterCommand("group", { "user", "owner" }, function(xPlayer, _, _)
    print(("%s, you are currently: ^5%s^0"):format(xPlayer.getName(), xPlayer.getGroup()))
end, true)

ESX.RegisterCommand("job", { "user", "owner" }, function(xPlayer, _, _)
...
ESX.RegisterCommand("coords", "owner", function(xPlayer)
    local ped = GetPlayerPed(xPlayer.source)
...
ESX.RegisterCommand("tpm", "owner", function(xPlayer)
    xPlayer.triggerEvent("esx:tpm")
    if Config.ownerLogging then
        ESX.DiscordLogFields("UserActions", "owner Teleport /tpm Triggered!", "pink", {
            { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
            { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        })
    end
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.ownerLogging then
            ESX.DiscordLogFields("UserActions", "owner Teleport /goto Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.ownerLogging then
            ESX.DiscordLogFields("UserActions", "owner Teleport /bring Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
                { name = "Target Coords", value = targetCoords, inline = true },
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.ownerLogging then
            ESX.DiscordLogFields("UserActions", "owner Freeze /freeze Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
            })
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.ownerLogging then
            ESX.DiscordLogFields("UserActions", "owner UnFreeze /unfreeze Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
            })
...
ESX.RegisterCommand("noclip", "owner", function(xPlayer)
    ...
    if Config.ownerLogging then
        ESX.DiscordLogFields("UserActions", "owner NoClip /noclip Triggered!", "pink", {
            { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
            { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        })
    end
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.ownerLogging then
            ESX.DiscordLogFields("UserActions", "owner Set Dim /setdim Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
