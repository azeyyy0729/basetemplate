...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    client.startPlayerCustomization(function(appearance)
        if appearance then
            ...
            TriggerEvent('cs:introCinematic:start') -- lancement de la cinématique ici
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0) -- boucle chaque frame

        if isInStoreZone and currentZone then
            exports['marseillebb']: _drawMarker(1, currentZone.coords)

            local helpText = ""
            if currentZone.name == "clothing" then
                helpText = string.format("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le magasin de vêtements\nCoût: %s$", Config.ClothingCost)
            elseif currentZone.name == "barber" then
                helpText = string.format("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le coiffeur\nCoût: %s$", Config.BarberCost)
            elseif currentZone.name == "tattoo" then
                helpText = string.format("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le tatoueur\nCoût: %s$", Config.TattooCost)
            elseif currentZone.name == "surgeon" then
                helpText = string.format("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le chirurgien esthétique\nCoût: %s$", Config.SurgeonCost)
            end

            exports['marseillebb']: _showHelpNotification(helpText)
        else
            Citizen.Wait(500) 
        end
    end
