...
local function wearClothes(data, typeClothes)
    ...
    local playerPed = PlayerPedId()
    local animationsOn = dataClothes.animOn
    local components = data.itemprops or dataClothes.props[client.getPedDecorationType()]
    local appliedComponents = data.components
    local clothesType = data.clothesType or dataClothes.type
    local appliedProps = data.props

    RequestAnimDict(animationsOn.animDict)
    while not HasAnimDictLoaded(animationsOn.animDict) do
        Wait(0)
    end

    if clothesType == "components" then
    ...
    elseif clothesType == "props" then
        for i = 1, #components do
            local propId = components[i][1]
...
local function removeClothes(typeClothes)
    local dataClothes = constants.DATA_CLOTHES[typeClothes]
    local playerPed = PlayerPedId()
    local animationsOff = dataClothes.animOff
    local components = dataClothes.props[client.getPedDecorationType()]
    local clothesType = dataClothes.type
    RequestAnimDict(animationsOff.animDict)
    while not HasAnimDictLoaded(animationsOff.animDict) do
        Wait(0)
    end
    ...
    elseif clothesType == "props" then
        for i = 1, #components do
            ClearPedProp(playerPed, components[i][1])
        end
    end


    TaskPlayAnim(playerPed, animationsOff.animDict, animationsOff.animName, 3.0, 3.0, animationsOff.length, 50, 0, false, false, false)
end
