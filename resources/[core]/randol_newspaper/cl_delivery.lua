...
local function validateDrop(point)
    ...
    if success then
        ...
        if num > 0 then
            DoNotification(('Journal livré. %s restant'):format(num))
        end
    end
    Wait(1000) 
    delay = false
...
local function createPaperRoute(netid)
    ...
    end, 'Impossible de charger le véhicule à temps.', 5000)
    ...
    clockedIn = true
    DoNotification('Vos lieux de livraison ont été assignés.', 'success')
end
...
if GetResourceState('es_extended') == 'started' then
    RegisterNetEvent('esx:removeInventoryItem', function(item, count)
        if item == 'WEAPON_ACIDPACKAGE' and clockedIn and count == 0 then
            DoNotification('Vous n’avez plus de journaux.')
            resetJob()
        end
    end)
else
