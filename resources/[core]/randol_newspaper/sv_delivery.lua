...
lib.callback.register('randol_paperboy:server:validateDrop', function(source, location, netid)
    ...
    if NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(GetPlayerPed(source))) ~= netid then
        payout = 1
        DoNotification(src, 'Ceci est le mauvais véhicule. Salaire réduit', 'error')
    end

    workers[src].totalPay += payout

    DoNotification(src, ('$%s ajouté à votre salaire total. Nouveau total : $%s'):format(payout, workers[src].totalPay), 'success')
...
lib.callback.register('randol_paperboy:server:clockOut', function(source)
    if not workers[source] then
        DoNotification(source, 'Vous n’avez aucune livraison active.', 'error')
        return false 
    end
    ...
    if workers[src].totalPay > 0 then
        ...
        DoNotification(src, ('Vous avez reçu $%s pour avoir terminé vos livraisons.'):format(workers[src].totalPay), 'success')
    end
