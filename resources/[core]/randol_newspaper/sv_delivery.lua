...
lib.callback.register('randol_paperboy:server:validateDrop', function(source, location, netid)
    ...
    if NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(GetPlayerPed(src))) ~= netid then
        payout = 1
        lib.callback.await('basic:ShowNotify', src, "Ce n'est pas le bon véhicule. Paiement réduit.")
    end

    workers[src].totalPay += payout

    lib.callback.await('basic:ShowNotify', src, ('%s$ ajoutés à votre paie. Total actuel : %s$'):format(payout, workers[src].totalPay))
...
lib.callback.register('randol_paperboy:server:clockOut', function(source)
    if not workers[source] then
        lib.callback.await('basic:ShowNotify', source, "Vous n'avez pas de livraison en cours.")
        return false
    end
    ...
    if workers[src].totalPay > 0 then
        ...
        lib.callback.await('basic:ShowNotify', src, ('Vous avez reçu %s$ pour avoir terminé vos livraisons.'):format(workers[src].totalPay))
    end

    if DoesEntityExist(workers[src].entity) then
        DeleteEntity(workers[src].entity)
    end
